...
export default function CryptoSystem({ profile, btcPrice, ethPrice, solPrice, suiPrice, onTradeCompleted }: CryptoSystemProps) {
  ...
  const handleAssetSelect = (asset: any) => {
    ...
    const sym = typeof symbol === 'string' ? symbol : symbol.toString();
    setSelectedSymbol(sym);
    setSelectedAssetForDetail({
      ...asset,
      asset_id: sym,
      name: asset.name || sym,
      price_usd: asset.price_usd || asset.currentPrice || 0,
      image: asset.image || undefined,
      price_change_24h: asset.price_change_24h || 0,
      market_cap: asset.market_cap || 0,
      coingeckoId: asset.coingeckoId || asset.id || sym.toLowerCase(),
    });
    setShowAssetDetail(true);
  };

  const handleAssetCardClick = (assetSymbol: string) => {
  ...
  return (
    <div className="space-y-8 md:space-y-12 pb-24 px-1 md:px-0">
      ...
      <AnimatePresence>
        {showAssetDetail && selectedAssetForDetail && (
          <AssetDetailView
              ...
              <MarketMetadata symbol={selectedSymbol} />
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {Array.isArray(assets) && assets.slice(0, 3).map((asset) => (
                    <div key={asset.symbol} className="bg-white border border-gray-100 rounded-[30px] p-8 space-y-6 shadow-xl hover:shadow-2xl transition-all cursor-pointer group" onClick={() => handleAssetCardClick(asset.symbol)}>
