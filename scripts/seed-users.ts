    ...
    try {
      ...
      try {
        await axios.post(`${API_BASE_URL}/sync/user`, {
          supabaseId: firebaseUser.uid, // Using firebase UID as supabaseId for compatibility
          ...user.profile,
          email: user.email,
          role: user.role
        });
        ...
        console.warn(`⚠️ MongoDB Sync Failed for ${user.email}. Make sure server is running.`);
      }
